...
const stripBookTitleMarks = (value) => String(value || "").trim().replace(/^《\s*|\s*》$/g, "");
        ...
        <section class="document-card">
          <header class="doc-head">
            <div class="doc-title">
              ...
              <div>
                <h2>${escapeHtml(stripBookTitleMarks(warehouse.reviewDocument.title))}</h2>
              </div>
            </div>
            ${renderToolbar()}
            <div class="chips">
...
function confirmCreateWarehouse() {
  ...
  const name = stripBookTitleMarks(input?.value);
  if (!name) {
